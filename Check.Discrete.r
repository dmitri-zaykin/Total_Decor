...
#pwt.YY <- pwt.XX <- rep(0, oo)
Y <- vec.1 <- rep(1, p)
for(j in 1:oo) {
    # repeat {
    #     R[lower.tri(R)] <- runif( p*(p-1)/2, -1, 1)
    #     R <- (R * lower.tri(R)) + t(R * lower.tri(R))
    #     diag(R) <- 1
    #     R <- as.matrix(nearPD(R, corr=TRUE, posd.tol = ptol)$mat)
    #     Beta <- runif(p, -0.1, 0.1) # random coefficients
    #     Cors <- c((Beta %*% R) / sqrt(as.numeric(c(Beta) %*% R %*% Beta + s^2)))
    #     x <- Cors; s.R <- t.R <- t.V <- R
    #     for(i in 1:p) {
    #         for(v in 1:p) {
    ...
    # Nonc <- ( (sqrt(N)*Cors) %*% eivec %*% sD )^2
    # crit.val.m <- Finv(alpha, ee$values)
    # pwt.YY[j] <- imhof(crit.val.m, lambda = eigva, delta = Nonc, epsrel = 1e-11, limit = 2e5)$Qq
    # pwt.XX[j] <- 1 - pchisq(qchisq(1-alpha, df=p), df = p, ncp = sum(Nonc))
    ...
    if(j %% 10 == 0) cat(file="/dev/stdout", ".", j, ".", sep=""); if(j %% 100 == 0) cat(file="/dev/stdout", "\r")
}
cat("\n")
( e.yy <- ecdf(pwe.YY)(alpha) ) ; mean(pwt.YY)
( e.xx <- ecdf(pwe.XX)(alpha) ) ; mean(pwt.XX)
